...
export default class DropdownAlert extends PureComponent {
    static defaultProps = {
        onClose: () => {},
        onCancel: () => {},
        closeInterval: 4000,
    ...
    _onDonePan = (event, gestureState) => {
        const start = this.getStartDelta(
            this.state.height,
            this.props.startDelta,
        );
        const delta = start / 5;
    ...
    open = (data = {}, duration) => {
        this.alertData = data;
        this.setState({ isOpen: true });
        this.updateStatusBar(this.props.updateStatusBar, true);
        this.animate(1, 450, () => {
            this.animationLock = false;
            if (duration > 0) {
                this.closeAutomatic(duration);
